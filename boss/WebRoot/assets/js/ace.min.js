...
ace.general_things = function(a) {
	...
	a("#btn-scroll-up").on(ace.click_event, function() {
		var c = Math.min(400, Math
						.max(100, parseInt(a("html").scrollTop() / 3)));
		a("html,body").animate({
					scrollTop : 0
				}, c);
		return false
	...
	a("#skin-colorpicker").on("change", function() {
		...
		if (d == "skin-2") {
			...
			a(".ace-nav > li:not(:last-child)").removeClass("light-pink")
...
ace.widget_boxes = function(a) {
	...
	a(document).on("click.ace.widget", "[data-action]", function(o) {
		...
		if (p == "collapse") {
			...
			if (l.length == 0) {
				g = g.wrapInner('<div class="widget-body-inner"></div>')
...
ace.switch_direction = function(d) {
	...
	c
			...
			.removeClass("pull-left").addClass("pull-right").end()
			.find(".tmp-rtl-pull-right").removeClass("tmp-rtl-pull-right")
			.addClass("pull-left").end().find(".chosen-container")
			.toggleClass("chosen-rtl").end();
	function a(h, g) {
