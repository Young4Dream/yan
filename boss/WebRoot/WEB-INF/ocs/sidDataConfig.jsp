	...
	<body>
					...
					<ul class="nav nav-tabs padding-18">
						<li class="active">
							<a data-toggle="tab" href="#ne"> <i
								class="green icon-youtube bigger-120"></i> 带宽列表 </a>
						</li>
					</ul>
					...
					<div class="modal-body">
						<form class="form-horizontal">
							<div class="form-group">
								...
								<label class="col-xs-2 control-label no-padding">
									<font color="green"><b>带宽（KB）：</b></font>
								</label>
								<div class="col-xs-4">
									<input type="text" id="bandWidth" name="obj.bandWidth" class="" style="width:93%;"/>
								</div>
							</div>
							<div class="form-group">
								<label class="col-xs-2 control-label no-padding">上行速率（KB）：</label>
								<div class="col-xs-4">
									<input type="text" id="upSpeed" name="obj.upSpeed" class="" style="width:93%;"/>
								</div>
								<label class="col-xs-2 control-label no-padding">下行速率（KB）：</label>
								<div class="col-xs-4">
									<input type="text" id="downSpeed" name="obj.downSpeed" class="" style="width:93%;"/>
								</div>
							</div>
						</form>
					</div>
        ...
        $("#save").click(function() {
        	...
        	var parm = {
    			"obj.id": $("#id").val(),
    			"obj.name": $("#name").val(),
    			"obj.bandWidth": $("#bandWidth").val(),
    			"obj.upSpeed": $("#upSpeed").val(),
    			"obj.downSpeed": $("#downSpeed").val()
        	};
        	var flag = $("#flag").val();
        	
        	var resData;
        	if (flag == "add") {
        		resData = ajaxCRUD("sidDataConfig!insert.do", parm, true);
        	} else {
        		resData = ajaxCRUD("sidDataConfig!update.do", parm, true);
        	}
        	if (resData.resNum == "0") {
        ...
        function del(id) {
	   		bootbox.confirm({
				...
				callback: function(result){
	   				if(result){
						var parm = {"id":id };
					    var resData = ajaxCRUD("sidDataConfig!delete.do", parm, true);
						if(resData.resNum == "0"){
							table_view.fnSettings().ajax.data = {}; 
							table_view.api().ajax.reload();
						}
        ...
        function detail(obj) {
        	...
        	$("#upSpeed").val(obj.upSpeed);
        	$("#downSpeed").val(obj.downSpeed);
        }
