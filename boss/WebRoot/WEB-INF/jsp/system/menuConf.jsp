...
<html>
	...
	<body>
		...
		function  buildSelectMenu(resData){
			...
			$(menuSelect).each(function(index) {
				var menuInfo = menuSelect[index];
				menuHtml += "<option role=\"option\" value=\""+menuInfo.menuId+"\">"+menuInfo.menuName+"</option>";
			});
    ...
    [{
		...
		jQuery(function($){
        	...
        	//$("#xx").html(JSON.stringify(ra).replace(/\"/g,"'"));    //
			data = JSON.stringify(ra).replace(/\"/g,"'");
        	
			$('#nestable').nestable().on('change', function(){ 
            	var r = $('.dd').nestable('serialize'); 
            	//$("#xx").html(JSON.stringify(r).replace(/\"/g,"'"));    //改变排序之后的数据
		...
		function updateWindow(menuId) {
			...
			var menuInfo= resData.menuInfo;
			clearWindow();
			$("#modalSubmit").text("更新");
			$("#modalId").val(menuInfo.menuId);
        	$("#modalName").val(menuInfo.menuName);
        	$("#modalDesc").val(menuInfo.menuDesc);
        	$("#modalIco").val(menuInfo.menuIco);
        	$("#modalUrl").val(menuInfo.menuUrl);
        	$("#modalParent").val(menuInfo.parentId);
        	$("#modalSort").val(menuInfo.menuSort);
        	$("#modalRemark").val(menuInfo.remark);
        	$("#modalCreateTime").val(menuInfo.createTime);
        	flag = 2;//更新flag
    	}
		...
		$("#modalSubmit").click(function(){//save
				if(flag==2){
					fromSubmin("menu!updateMenuInfo.do",flag);
				}else if(flag==1){
		...
		function fromSubmin(url,flag) {
        	...
        	if(flag==1){
        		params = {
							"menuInfo.menuName":rname,
        					"menuInfo.menuDesc":rdesc,
        					"menuInfo.menuIco":rico,
        					"menuInfo.menuUrl":rurl,
        					"menuInfo.parentId":rParent,
        					"menuInfo.menuSort":rSort,
        					"menuInfo.remark":rremark,
        					"menuInfo.createTime":rct
						};
        	}else{
				params = {
							"menuInfo.menuId":rid,
        	...
        	bootbox.confirm({
    			...
    			callback: function(result){
        			if(result){
						...
						var resData = ajaxCRUD("menu!deleteMenuInfo.do",params,true);//url，参数，是否弹出alert
						$("#modalParent").text("");
  						buildSelectMenu(resData);
						rebuildMenu(resData);
						var ra = $('.dd').nestable('serialize'); 
		...
		function buildMenu(resData){
			...
			$(menuList).each(function(index) {
				...
				//alert(menu);
				if(menu.parentId==0){
					buildMenuChild(menu.menuId,menu.menuName,menu.parentId,menuList);
				}
          	});
			html += "</ol>";
			...
			$("#nestable li").smartMenu(imageMenuData,{
				afterShow:function() {
					...
					//parentID用id记录  格式为menuId:x-parentId:x 需要解析
		...
		//创建menu子节点
		function buildMenuChild(menuId,menuName,parentId,menuList){
			html += "<li id=\"menuId:"+menuId+"-parentId:"+parentId+"\"class=\"dd-item\" data-id="+menuId+" >"+
					"<div class=\"dd-handle\">"+menuName+"</div>" ;
			if(hasChild(menuId,menuList)>0){
